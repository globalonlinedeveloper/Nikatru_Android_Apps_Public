...
`DioApiClient.getEntitlements()` (`apps/subscriptiontracker/lib/data/api/dio_api_client.dart:113`,
`GET /entitlements` on this Worker's base) wrapped by
`SubscriptionRepository.entitlements()`
(`apps/subscriptiontracker/lib/data/subscriptions/subscription_repository.dart:20`) — and it
...
(`apps/subscriptiontracker/lib/state/money_providers.dart:126`) → `entitlementTransportProvider`
...
`name` is `subscriptiontracker-api`, `entrypoint` `services/subscriptiontracker-api/src/index.ts` and `config`
`services/subscriptiontracker-api/wrangler.jsonc`, with a TWELVE-entry `routes[]` —
`subscriptiontracker-renewals` and `subscriptiontracker-entitlements` among them — and
...
run: `⚠  GET /v1/renewals — NO CLIENT. · subscriptiontracker-api 🔴 NOTHING IN THIS REPO CALLS
IT`.
...
- **the route set**, both directions — every route `services/subscriptiontracker-api/src/index.ts`
...
two routes — `subscriptiontracker-entitlements`, `client.expression` `_rest.get('/entitlements')`
...
itself, and the `subscriptiontracker-entitlements` entry then records BY HAND what the machine
...
subscriptiontracker-api no longer carries its own cron trigger. The nightly scheduler
