// ─────────────────────────────────────────────────────────────────────────────
// health.test.ts — THE RECORDED FAILING CASES FOR `/v1/health` ON subscriptiontracker-api.
//
...
const APP_DB_SECRET = 'D1_ERROR: connect ECONNREFUSED subscriptiontracker-app-db.internal:5432 token=sk_live_7c21';
...
describe('/v1/health — the healthy answer keeps the OLD shape', () => {
  it('answers ok:true, 200, and the fields the deploy smoke joins on', async () => {
    ...
    expect(body.app).toBe('subscriptiontracker');
...
describe('🔴 APP_DB unreachable — the endpoint says NO', () => {
  ...
  it('⛔ leaks no host, port, token or vendor error text', async () => {
    ...
    expect(text).not.toContain('subscriptiontracker-app-db.internal');
