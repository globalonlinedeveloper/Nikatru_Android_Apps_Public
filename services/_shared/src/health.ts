// ─────────────────────────────────────────────────────────────────────────────
// health.ts — THE MACHINERY THAT LETS `/v1/health` SAY NO. THE ONE HOME.
//
// 🔴 THE MEASURED STATE THIS REPLACES. Every Worker answered `/v1/health` with
...
// 401s) and BOTH stayed green. A stamped app inherited the same day-one hole.
// That is the `Ping` monitor shape at the deepest point in the stack: it proves
// a socket opened and nothing else.
//
...
// the hashes were telling anyone. Every carrier now RE-EXPORTS this file, and
// the twinned test asserts that it does.
//
// ⚠️ NOTHING IN `services/_shared/src/` MAY CARRY A BARE IMPORT, and that is a
...
// Wrangler's own advice there — add an `alias` entry — is REFUSED: an alias to
// `./node_modules/jose` resolves a DIRECTORY and bypasses the package's
// `exports` conditions, which is exactly how `vitest.config.ts` (see its 40-line
// header) already lost the `workerd` build of jose once. So the rule is: this
// directory is dependency-free, and a module that needs `jose` or `hono` keeps
// its plumbing in the Worker and shares only what is pure — see
// `services/_shared/src/auth.ts`.
//
// This module needs nothing: `fetch`, `setTimeout` and `Map` are runtime
// globals. Nothing Worker-specific may live here either — the dependency LIST
// is assembled in each Worker's `src/index.ts`, which is where the carriers
// legitimately differ.
//
