...
| `support-url.txt` | Support contact — website | `apps/subscriptiontracker/lib/core/app_config.dart` → `contactUrl` | ✅ exact match |
...
| **Renovate bumps a plugin whose plugin manifest was read** | `androidPermissions.merged.pinned` is compared to `pubspec.lock` by **limb 7a**. A version bump is the ONE way the merged-permission measurement rots with **no other file in this repository changing** — the pubspec range did not move, so neither the manifest equality nor the dependency equality can see it. Re-take the reading from a fresh `.aab` |
| **A permission recorded as `expectedButAbsent` appears in the merged set** | limb 7a again. `ACCESS_NETWORK_STATE` was *predicted* by reasoning about the plugin list and *measured absent* off the built artefact; if a later `sentry-android` brings it in, the block contradicts its own receipt and fails, which forces somebody to say what the new permission changes instead of appending it silently |
| **A merged row loses the evidence its own grade promises** | limb 7a grades every row: `read` and `inferred` must carry their evidence, an `inferred` row must carry the `residual` that is the whole difference between the two, and an `unattributed` row must have a matching named finding under `merged.unattributed` — **both directions** |
| A new direct dependency appears | *"nobody said what data it can collect"*. 🔴 This cell read *"the mitigation for not being able to read the merged manifest"* until 2026-08-26; the merged manifest **has** been read since, off a signed `.aab`, and limb 7a asserts that reading. The mitigation narrowed rather than disappeared: nothing in CI re-downloads its own `.aab`, so between measurements this equality is still what catches a permission-bearing SDK arriving |
...
- ✅ **Target API level** — Android 16 / **API 36 by 2026-08-31**. A build property, not a
  listing field, and **already met**: `apps/subscriptiontracker/android/app/build.gradle.kts` pins the
...
node tooling/release/submit-play.mjs --dry-run --app subscriptiontracker
```
