...
| `privacy-manifest.json` | *(not a listing field — see below)* | **measured from the tree** on 2026-08-31: the plugin closure, each plugin's own `PrivacyInfo.xcprivacy`, and `apps/subscriptiontracker/store/android-play/data-safety.json` for the collected-data rows | ✅ `tooling/ci/assert-apple-privacy-manifest.mjs` re-derives **both** `PrivacyInfo.xcprivacy` files from it and compares |
...
`apps/subscriptiontracker/ios/Runner/PrivacyInfo.xcprivacy` and
`apps/subscriptiontracker/macos/Runner/PrivacyInfo.xcprivacy`, and that
...
| `NSPrivacyCollectedDataType*` and `…Purpose*` spellings | ⚠️ **UNVERIFIED — no local witness** | every plugin manifest in the closure ships `NSPrivacyCollectedDataTypes` **empty**, so not one of these constants appears in any file on this machine. They are cross-checked against `apps/subscriptiontracker/store/android-play/data-safety.json` instead of being trusted alone — a misspelling is a plist Apple *ignores*, not an error it reports |
