...
| `privacy-policy-url.txt` | Privacy policy link | `apps/subscriptiontracker/lib/core/app_config.dart` → `privacyUrl` | ✅ exact match |
| `support-url.txt` | Contact / support link | `apps/subscriptiontracker/lib/core/app_config.dart` → `contactUrl` | ✅ exact match |
...
⚠️ `subscriptiontracker` is the name this repo *intends*. **Whether it is available is
...
```
node tooling/store/render-linux-icons.mjs --app subscriptiontracker
```

writes `apps/subscriptiontracker/linux/packaging/` — a freedesktop `com.nikatru.subscriptiontracker.desktop`
...
node tooling/release/submit-snap.mjs --dry-run --app subscriptiontracker
```
