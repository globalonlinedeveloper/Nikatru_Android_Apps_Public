...
class SublyApp extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return MaterialApp.router(
      ...
      // `AppTheme.light()` because "apps/subscriptiontracker is a frozen legacy rail-prover
      ...
      // the warning named — is read ZERO times in apps/subscriptiontracker/lib; the only
      ...
      // deleting `themeMode:` or either `seed:` reddens them. `apps/subscriptiontracker` is in
      ...
      // AnalyticsGate rather than a `showDialog`, and it is why the older
      // dialog-shaped `ConsentGate` — which borrowed the root Navigator's
      // context via `rootNavigatorKey` — was retired here. See MANIFEST
      // decision D-4. It kept COMPILING for another two months after it stopped
...
class _AnalyticsGateState extends ConsumerState<AnalyticsGate>
  ...
  Widget build(BuildContext context) {
    ...
    // WHAT BUY BACK WHAT `ModalRoute` USED TO GIVE FREE — measured, not
    // assumed. Before them, with the prompt up, a walk of the compiled
    // semantics tree (`tester.ensureSemantics()`, real `SublyApp`) found 15
    // labelled nodes and the onboarding carousel BEHIND the scrim fully
    // exposed: "Every subscription, one clean board", "Page 1 of 3", "Skip",
    // "Next", the six category chips — with `Skip` and `Next` carrying live tap
    // actions. Semantic taps dispatch straight to the widget and DO NOT
    // hit-test, so the opaque `ColoredBox` stops a finger and stops nothing for
    // TalkBack or VoiceOver: a screen-reader user could drive onboarding
    // underneath a modal they were never told they were inside.
    //
            ...
            child: Padding(
              ...
              child: ConstrainedBox(
                ...
                child: Material(
                  ...
                  // text is large. Measured on the real `SublyApp` at the
                  // largest text this chassis PERMITS (`maxScaleFactor: 2.0`,
                  // :113 — 2.0 is in range by design, not an extreme):
