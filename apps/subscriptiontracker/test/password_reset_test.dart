...
ProviderContainer _container(
  ...
  String launchUrl = 'https://nikatru.com/subscriptiontracker/',
}) => ProviderContainer(
  overrides: <Override>[
...
void main() {
  ...
  testWidgets('the PKCE crash [SUBLY-8] becomes a sentence on this screen', (
    ...
    final ProviderContainer c = _container(
      auth,
      launchUrl:
          'https://nikatru.com/subscriptiontracker/?nk_auth=reset&code=abc123#/reset-password',
    );
    addTearDown(c.dispose);
    await _pump(tester, c);
  ...
  testWidgets('leaving CLEARS the arrival, so a dead link is not a trap', (
    ...
    final ProviderContainer c = _container(
      auth,
      launchUrl:
          'https://nikatru.com/subscriptiontracker/?nk_auth=reset#error=access_denied'
  ...
  testWidgets('an OAuth callback with a code is NOT sent here', (
    ...
    final ProviderContainer c = _container(
      auth,
      launchUrl: 'https://nikatru.com/subscriptiontracker/?code=an-oauth-code',
    );
    addTearDown(c.dispose);
