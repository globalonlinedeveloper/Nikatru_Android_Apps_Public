...
/// The brand widgets were lifted out of `apps/subscriptiontracker/lib/features/shared/
/// widgets.dart` into `packages/design_system` (`BrandWordmark` /
/// `BrandFooter`), and the app's copies became thin adapters that supply
/// `AppConfig` + the arb strings. `apps/subscriptiontracker` is the app the owner eyeballs,
...
/// are not bundled (see `apps/subscriptiontracker/pubspec.yaml`'s commented `fonts:` block),
...
const String _kWhy =
    'apps/subscriptiontracker is the app the owner eyeballs. Lifting these widgets into '
...
Text(Subscriptions by Nikatru ff=Manrope fw=500 fs=12.0 color=#ff6f6f7b dec=null)
SizedBox(w=null h=8.0)
...
Image(asset=assets/brand/nikatru-logo.png h=22.0 fq=medium sem=Nikatru)
SizedBox(w=null h=8.0)
Text(Subscriptions by Nikatru ff=Manrope fw=500 fs=12.0 color=#ffc8c5d0 dec=null)
SizedBox(w=null h=8.0)
Wrap(align=center cross=center sp=0.0 run=0.0)
FocusableTap(role=link behavior=deferToChild radius=4.0 focusColor=null merge=true label=null enabled=true)
