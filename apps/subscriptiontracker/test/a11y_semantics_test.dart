...
/// MEASURED 2026-08-21, `rg "RowCard\("` over `apps/subscriptiontracker/lib`: the widget is
  ...
  group('contrast · flutter_test\'s own text-contrast sweep', () {
    ...
    testWidgets('every string on home meets WCAG AA contrast', (
      ...
      await semantically(tester, () async {
        ...
        // the only three `DueInfo.localized(` CALL SITES under `apps/subscriptiontracker/lib`
    ...
    test('the due-label urgent branch clears AA in BOTH brightnesses', () async {
      ...
      // `grep -n "except:" apps/subscriptiontracker/test/a11y_semantics_test.dart` returns one
      ...
      // callers were all assertions in `apps/subscriptiontracker/test` — so there is no second
      ...
      // `grep -rnF 'DueInfo.localized(' --include=*.dart apps/subscriptiontracker/lib` returns
