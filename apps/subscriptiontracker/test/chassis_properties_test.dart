...
void main() {
  ...
  group('property: brand-seed-drives-paint', () {
    ...
    testWidgets('this app really paints with ITS OWN seed', (
      ...
      final ThemeData expected = buildAppTheme(seed: const Color(0xFF6459F5));
      expect(
  ...
  group('property: ui-invariants-inherited', () {
    ...
    testWidgets('every icon-only control carries a screen-reader label', (
      ...
      try {
        ...
        // RETARGETED FOR SUBLY (P2.6b) — same property, different widget:
        // under VARIANT B the navigation destinations are the AppDestination
        // list AppShell hands the chassis AppScaffold — the single source the
        // pill, rail and drawer all derive from. NavigationDestination is a
        // widget this tree never builds (compact renders the custom pill via
        // the compactNavigationBar seam), so the old finder ranged over an
        // EMPTY set — a vacuous pass, which is worse than a red. Not a
        // weakening: the domain goes from 0 widgets to the 5 real destinations.
        final List<AppDestination> found = tester
        ...
        for (final AppDestination d in found) {
          expect(
            d.label.trim(),
            isNotEmpty,
            reason: 'an unlabelled icon is unusable with a screen reader',
          );
          // The compact pill really renders each label — the destinations
          // list being labelled proves nothing if the renderer drops them.
  ...
  group('property: auth-redirect-follows-session', () {
    testWidgets('signing in through the FORM moves the user off it', (
      ...
      // Landing on the form is itself a NAVIGATION in this app: the union
      // router opens on '/onboarding' and moves to the form only after the
      // onboarding flag hydrates, so the route change must be allowed to finish
      // before the form is driven.
      await _turnsAndSettleRoute(tester);
      ...
      // The union's signed-out form is the live LoginScreen, whose submit is a
      // GradientButton (an InkWell), not the stamp's FilledButton. Same property,
      // driven through the real control — the E2E key exists exactly for this.
  ...
  group('property: onboarding-shown-once', () {
    ...
    testWidgets('it never shows a raw config key to a user', (
      ...
      // literal; the arb key `subscriptiontrackerOnboarding1Title` that replaced it is
      ...
      // becoming `l10n.subscriptiontrackerOnboarding1Title`: the property under test is that a
  ...
  group('property: paywall-gate-driven-by-server', () {
    ...
    test(
      '[5]M-8 · a stale answer RE-LOCKS, and connectivity does not excuse it',
      () async {
        final ProviderContainer c = _moneyContainer(
          store: _onboardedStore(),
        ...
        // 🔴 BOTH DIRECTIONS. Without the first, a client that ALWAYS locks
    ...
    testWidgets('the GATE really covers the premium surface', (
      ...
      // anything is a wall, not a paywall. The union gate lives on the
      // router's /insights branch (_GatedInsights, lib/core/router.dart), and
      // StatefulShellRoute.indexedStack builds branches lazily — so on the
      // first surface the gate is not merely quiet, it is absent.
