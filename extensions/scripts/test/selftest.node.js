...
const NO_CASE_RECORDED = [
  ...
  { gate: 'scripts/test/hang-guard.test.mjs',
    why: 'PERMANENT, recorded 2026-09-06. Like this file, it is a suite rather than a gate — node:test, run by the same `selftest` job. What grades it is that each of its four cases is a proven pair, and one of them found a real defect in the guard on the day it was written: the SIGKILL the ceiling issues raises its own `exit` event, which beat the ceiling to the verdict and reported a wedged process as exit 1 instead of 124.' },
  { gate: 'scripts/lib/hang-guard.mjs',
    why: 'COVERED ELSEWHERE, recorded 2026-09-06 — this is not an open gap. Its red/green pairs are in scripts/test/hang-guard.test.mjs (a command exiting 0 passes through as 0; one that never exits is killed at the ceiling and reported 124, with Node\'s diagnostic report on disk to prove the signal landed; one exiting 3 comes back as 3, not 1 and not 124), and the `selftest` job runs them in this same job. They cannot live HERE: run() appends `--repo-root <root>`, the guard forwards everything after `--` to the child verbatim, and `node -e ... --repo-root x` answers "bad option: --repo-root" and exits 9 — measured 2026-09-06. So every case through this file\'s runner would grade node\'s argument parser, not the ceiling.' },
