...
/// rules live.
///
/// 🏗️ THE BODY OF `LegalConsentFields`, MOVED HERE BY [ADR 067] decision 2 /
/// [ADR 071]. The brick keeps an adapter of the same name that supplies
/// `AppConfig.termsUrl` / `privacyUrl` and opens them with `url_launcher`.
/// THAT SPLIT IS THE POINT AND NOT A CONVENIENCE: declaring `url_launcher`
/// here would make this package an ADAPTER under
/// `assert-package-boundaries.mjs` limb C (`:185-201`), every third-party name
/// it declared would be recorded as a WRAPPED vendor, and `:288-298` would then
/// fail every app and the brick for importing `url_launcher` directly. So the
/// links arrive as [onOpenTerms] / [onOpenPrivacy] callbacks and this file
/// knows no URL at all.
///
...
/// sign-up surface — in this template, in any app, or in the chassis file one of
/// them delegates to — initialises either flag to true. Pre-ticked or
/// uncheckable consent is dark-pattern territory under Planet49/EDPB, DPDP
/// Rules 2025 and CPRA alike.
///
...
class LegalConsentFieldsView extends StatelessWidget {
  ...
  /// Opens the LIVE terms page. A callback rather than a URL: see the header.
    ...
    return Column(
      ...
      children: <Widget>[
        _Row(
          ...
          links: <Widget>[
            _LegalLink(l10n.termsOfService, onOpenTerms),
            _LegalLink(l10n.privacyPolicy, onOpenPrivacy),
          ],
        ),
...
class _LegalLink extends StatelessWidget {
  const _LegalLink(this.label, this.onOpen);
