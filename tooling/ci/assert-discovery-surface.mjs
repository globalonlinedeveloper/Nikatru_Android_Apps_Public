...
// written.
//
// ⚠️ APPENDED 2026-08-25 — THIS FILE'S PAGE-QUALITY LIMBS NOW RANGE OVER TWO
// DEPLOY ROOTS, AND THAT DOES NOT CROSS THE BOUNDARY ABOVE. The accessibility
// limb, the served-<img> box limb and limb H (og:image) sweep
// PAGE_QUALITY_ROOTS — see the block where that constant is built for which
// limbs stayed on DEPLOY_ROOT and why. [1]F-9's check-site-integrity.mjs does
// range over both roots already, so "is this the same subject?" was MEASURED
// rather than assumed, 2026-08-25:
// `grep -n -iE 'skip link|skip-link|focus-visible|<main' tooling/ci/check-site-integrity.mjs`
// returns ZERO, and so does the same grep for `og:image`. Its subject there is
// canonical form and the sitemap↔page relation; none of these five properties
// is asserted by it, on either root. One fault still produces one message.
//
 ...
 *  Until then every limb in this file ranged over `DEPLOY_ROOT` alone.
 *
 *  🔴 THE SPLIT IS DELIBERATE AND IT IS NOT "two deploy roots everywhere". The
 *  GENERATED-ARTIFACT limbs — A (drift), B (the landing-set relationship), C
 *  (slots), D (JSON-LD), G (prices), the chrome-sentinel limb and the snapshot
 *  limb — STAY on DEPLOY_ROOT. The mirror is generated by nothing, spliced by
 *  nothing and priced by nothing, so pointing those limbs at it would fault a
 *  hand-written brochure for lacking artefacts it was never supposed to own.
 *  That is the recorded `chromeOnly` mistake — twelve findings about pages that
 *  were entirely correct — repeated one deploy root further out.
 *
 *  The PAGE-QUALITY limbs are the ones whose subject is WHAT A STRANGER GETS:
 *  the accessibility limb, the served-<img> box limb, and limb H's og:image
 *  block. Those properties are true or false of any served HTML regardless of
 *  who typed it, and sites/rajasekarselvam is served to strangers under the same
 *  Cloudflare Pages no-build-step arrangement — the bytes in this repository are
 *  the bytes a visitor gets, which is this whole file's stated posture.
 *  `sites/rajasekarselvam/404.html` is that site's CUSTOM 404, the page every
 *  wrong URL on the host lands on, and until 2026-08-25 it had no <main> and no
 *  `:focus-visible` rule and nothing in this repository could say so.
 *
 *  ⚠️ THE MIRROR IS GATED ON EXISTING, for the reason the CHROME_EXCLUDED audit
 *  is scoped to entries whose parent directory exists: a fixture tree that
 *  models only the nikatru deploy root has not LOST the mirror, it does not
 *  model it. That gate is a hole only if the REAL repository can lose the root
 *  quietly, so losing it there is refused by name immediately below.
 *  DEPLOY_ROOT is NOT gated — every limb above already refuses a tree without
 *  it, loudly. */
const MIRROR_ROOT = 'sites/rajasekarselvam';
const DECLARED_PAGE_QUALITY_ROOTS = [DEPLOY_ROOT, MIRROR_ROOT];
const PAGE_QUALITY_ROOTS = DECLARED_PAGE_QUALITY_ROOTS.filter(
  (root) => root === DEPLOY_ROOT || (existsSync(abs(root)) && statSync(abs(root)).isDirectory()),
);
if (SCANNING_OWN_REPO) {
  const lost = DECLARED_PAGE_QUALITY_ROOTS.filter((root) => !PAGE_QUALITY_ROOTS.includes(root));
  if (lost.length) {
    coverageLost([
      `${lost.length} declared page-quality root(s) are not directories in THIS repository: ${lost.join(', ')}.`,
      'Every page-quality limb below is gated on a root existing, so a root that vanishes takes its pages out',
      'of the accessibility, <img>-box and og:image contracts while this guard goes on printing a count. In a',
      'fixture tree an absent root is a tree that does not model it; here it is a deploy root that went',
      'missing. If a site was genuinely retired, delete it from DECLARED_PAGE_QUALITY_ROOTS in the same',
      'commit, naming what remains.',
    ]);
  }
}

/**
 * Served pages the page-quality contract does NOT apply to, BY NAME and with the
 * reason. This is the mirror-root counterpart of CHROME_EXCLUDED, and it costs
 * the same thing: a written reason, and an audit below that fails when the entry
 * outlives its subject or its premise.
 *
 * 🔴 THE PREMISE IS ASSERTED, NOT ASSUMED: every page excused here must still
 * carry `noindex`. What makes an unpolished page tolerable is that nothing sends
 * a stranger to it; the day it becomes indexable it owes the contract like every
 ...
 * reason. One condition on one page — never a page, never a condition.
 *
 * 🔴 SELF-RETIRING, and that is what separates this from a hole. The audit below
 * fails if an entry names a condition this limb did not evaluate (a typo, or a
 * page that left the sweep) AND if the named page has come to SATISFY the
 * condition it is excused from. So an exemption cannot outlive its subject, and
 ...
 *
 * DERIVED from the tree, isChromePage's reason. Under DEPLOY_ROOT the subject
 * set is exactly the chrome set, unchanged — the dated snapshots and the two
 * CHROME_EXCLUDED pages are out for the reasons written where they are named.
 * Under any other page-quality root every served .html is in, minus what
 * PAGE_QUALITY_EXCLUDED names with a reason. There is no third state in which a
 * page is quietly skipped.
 *
 * @param {string} rel repo-relative, POSIX-separated
 */
function isPageQualitySubject(rel) {
  if (!rel.toLowerCase().endsWith('.html')) return false;
  if (PAGE_QUALITY_EXCLUDED.has(rel)) return false;
  if (rel.startsWith(`${DEPLOY_ROOT}/`)) return isChromePage(rel);
  return PAGE_QUALITY_ROOTS.some((root) => rel.startsWith(`${root}/`));
}

/** Every served .html under a page-quality root, repo-relative and POSIX. */
function walkPageQualityRoots() {
  const found = [];
  for (const root of PAGE_QUALITY_ROOTS) {
    ...
    while (stack.length) {
      ...
      for (const e of listDir(dir, { withFileTypes: true })) {
        ...
        if (e.name.toLowerCase().endsWith('.html')) {
          found.push(`${root}/${f.slice(rootAbs.length + 1).split(sep).join('/')}`);
        }
      }
    }
  }
...
//
// ⚠️ APPENDED 2026-08-25 — THE DOMAIN ABOVE IS NO LONGER THE WHOLE OF IT, and
// the sentences above are left standing because they were true of the day they
// were written. This limb now sweeps PAGE_QUALITY_ROOTS, not DEPLOY_ROOT, and
// the second root is the hand-written mirror. It is NOT a chrome member (
// CHROME_ROOT is sites/nikatru, and tooling/sites/chrome.mjs says why), so the
// subject set there is `isPageQualitySubject`, not `isChromePage`. Measured that
// day with this limb's own matchers over the mirror's three served pages:
// index.html {lang ✓, mains 1, skip-link ✗ (its class is `skip`, excused by
// name), :focus-visible ✓, 1 served <img> with alt and a box};
// 404.html BEFORE the same day's content fix {mains 0, :focus-visible ✗, no skip
// link} — the CUSTOM 404 every wrong URL on that host reaches; cv.html
// {mains 0, :focus-visible ✗, no skip link}, excused by name as the mirror's
// _template.html.
//
...
let a11yChecked = 0;
let boxChecked = 0;
// 🔴 WHAT WAS AUDITED, NOT WHAT THE MAPS CONTAIN — house rule 2, and the first
// version of the ok line got it wrong. It printed `PAGE_QUALITY_EXCLUDED.size`,
// so a tree that models no mirror root would have printed "1 page(s) … excused
// by name, each still audited" over a run in which that entry was skipped
// entirely (both audits are scoped to entries whose parent directory exists).
// These two count the entries this run actually put a question to.
let pageExcusalsAudited = 0;
let conditionExcusalsAudited = 0;
{
  const found = walkPageQualityRoots();

  // Every condition this limb evaluates is routed through here, so that a
  // per-condition exemption is a NAMED entry with a reason and never an `if`
  // somebody added to a matcher. `evaluated` and `failed` are what the audit
  // below reads to prove an exemption is still needed; `msgs` empty means the
  // page satisfied the condition.
  const evaluated = new Set();
  const failed = new Set();
  const condition = (rel, cond, msgs) => {
    const key = `${rel}#${cond}`;
    evaluated.add(key);
    if (msgs.length === 0) return;
    failed.add(key);
    if (PAGE_QUALITY_CONDITION_EXCLUDED.has(key)) return;
    for (const m of msgs) problems.push(m);
  };

  for (const rel of found.filter(isPageQualitySubject)) {
    a11yChecked++;
    const html = readFileSync(abs(rel), 'utf8');
    ...
    condition(rel, 'lang', /<html[^>]+(?<![-\w:])lang\s*=\s*["'][a-z]{2}/i.test(html)
      ? []
      : [`${rel} has no <html lang="…">. A screen reader with no declared language reads the page in the user's default voice, which mispronounces every word of it.`]);
    ...
    // 🔴 THE `skip-link` CLASS TOKEN IS THE IDENTIFIER AND IS NOT TO BE WIDENED.
    // sites/rajasekarselvam/index.html spells its own `skip` and is excused BY
    // NAME in PAGE_QUALITY_CONDITION_EXCLUDED above, with the reversal written
    // there. Loosening this alternation instead would turn one page's red green
    // by making every page in the contract satisfiable with a class the shared
    // chrome never emits — a matcher repair that certifies, which is the defect
    // class the `\b` notes throughout this file record.
    if (!skip) {
      condition(rel, 'skip-link', [`${rel} has no skip link. Every page here opens with the same sticky nav, so without one a keyboard user tabs through all of it before reaching any content (WCAG 2.2 SC 2.4.1).`]);
    ...
    // skip link resolving to nothing, and `<div ids="main">` satisfied it.
      ...
      condition(rel, 'skip-link', new RegExp(`id\\s*=\\s*["']${skip[1]}["']`, 'i').test(html)
        ? []
        : [
    ...
    condition(rel, 'focus-visible', /:focus-visible/.test(html)
      ? []
      : [`${rel} declares no :focus-visible rule. A keyboard user cannot see where they are (WCAG 2.2 SC 2.4.7).`]);
    ...
    //
    // 🔴 ONE CONDITION, ONE MESSAGE PER OFFENDING TAG. The condition is
    // evaluated once per page (so an exemption could only ever excuse the whole
    // page's <img> set, never one tag of it), and every altless tag still gets
    // its own message so the reader is told WHICH image.
    //
    ...
    // has none.
    condition(rel, 'img-alt', (html.match(/<img(?![-\w])[^>]*>/gi) ?? [])
      .filter((tag) => !/(?<![-\w])alt\s*=/i.test(tag))
      .map((tag) => `${rel} has an <img> with no alt attribute: ${tag.slice(0, 90)}. An absent alt is read aloud as the filename; alt="" is the correct way to say "decorative".`));
  }

  // ── THE TWO EXEMPTION MAPS ARE AUDITED, and both audits can fail ──────────
  // An exemption that outlives its subject is an exemption nobody re-examines —
  // the CHROME_EXCLUDED audit's sentence, and it applies twice as hard here
  // because these excuse a PAGE-QUALITY property rather than a footer.
  //
  // Scoped to entries whose PARENT DIRECTORY exists, exactly as that audit is
  // scoped and for its reason: a tree with no sites/rajasekarselvam at all does
  // not model that area — every generator fixture in the suite is one — and
  // faulting it for an absent exemption subject would be reporting on a page the
  // tree never claimed to have.
  for (const rel of PAGE_QUALITY_EXCLUDED.keys()) {
    if (!existsSync(dirname(abs(rel)))) continue;
    pageExcusalsAudited++;
    if (!found.includes(rel)) {
      problems.push(
        `PAGE_QUALITY_EXCLUDED names ${rel}, which is not served from any page-quality root ` +
          `(${PAGE_QUALITY_ROOTS.join(', ')}). Delete the entry, or restore the page — a standing exemption ` +
          'for a file that no longer exists is a hole waiting for a future page to be dropped into it.',
      );
      continue;
    }
    if (!/<meta[^>]+name\s*=\s*["']robots["'][^>]*noindex/i.test(readFileSync(abs(rel), 'utf8'))) {
      problems.push(
        `${rel} is excused from the page-quality contract by PAGE_QUALITY_EXCLUDED, and it has LOST its ` +
          '`noindex`. The excuse rests on nothing sending a stranger there: it is served from a Cloudflare ' +
          'deploy root with no build step, so an indexable page carrying no <main>, no skip link and no ' +
          ':focus-visible rule is one a search result can now land somebody on. Restore the noindex, or ' +
          'delete the exemption and bring the page up to the contract.',
      );
    }
  }
  for (const key of PAGE_QUALITY_CONDITION_EXCLUDED.keys()) {
    const rel = key.slice(0, key.lastIndexOf('#'));
    if (!existsSync(dirname(abs(rel)))) continue;
    conditionExcusalsAudited++;
    if (!evaluated.has(key)) {
      problems.push(
        `PAGE_QUALITY_CONDITION_EXCLUDED names ${key}, and this limb evaluated no such condition on that ` +
          'page. Either the page left the sweep (deleted, or newly excused whole by PAGE_QUALITY_EXCLUDED) ' +
          'or the condition name is misspelled — and a misspelled key excuses nothing while reading exactly ' +
          'like an exemption that works. Fix the key or delete the entry.',
      );
      continue;
    }
    if (!failed.has(key)) {
      problems.push(
        `PAGE_QUALITY_CONDITION_EXCLUDED still excuses ${key}, and that page now SATISFIES the condition. ` +
          'Delete the entry: an exemption kept past its cause is one that silently covers the NEXT ' +
          'regression on the same page, which is the state this map exists to make impossible.',
      );
    }
  }
  ...
  //
  // ⚠️ APPENDED 2026-08-25, BECAUSE THE ARGUMENT ABOVE NOW NEEDS ONE MORE STEP
  // AND STILL HOLDS. The two counters are no longer EQUAL: this limb sweeps
  // `found.filter(isPageQualitySubject)` over PAGE_QUALITY_ROOTS while the
  // chrome limb sweeps `found.filter(isChromePage)` over DEPLOY_ROOT, so
  // `a11yChecked >= chromePagesChecked` with the mirror's subjects on top. The
  // conclusion is unchanged: `isPageQualitySubject` returns `isChromePage` for
  // everything under DEPLOY_ROOT, so `a11yChecked === 0` still IMPLIES
  // `chromePagesChecked === 0`, and the chrome limb's own refusal runs first and
  // exits. What that argument does NOT cover is the mirror half going empty on
  // its own — a root that stops being walked — and that is refused separately
  // and by name where PAGE_QUALITY_ROOTS is built, under SCANNING_OWN_REPO.
  ...
  //
  // ⚠️ (4) WAS CORRECTED 2026-08-25 AND IS LEFT STANDING SO THE CHANGE IS
  // VISIBLE. This limb now sweeps PAGE_QUALITY_ROOTS, which includes the mirror
  // when that root exists; (1), (1b), (2) and (3) are unchanged. Re-measured
  // that day with this limb's own walk and matchers, AFTER the same day's
  // content fix: 5 served <img> elements across the two roots — three shots at
  // 440x275 in sites/nikatru/apps/_template.html, the 150x192 founder photo in
  // sites/nikatru/index.html, and one in sites/rajasekarselvam/index.html which
  // already carries integer width+height. So the tree still complies and what
  // this widening adds is again ENFORCEMENT, not a repair. The 2026-08-21
  // sentence above — "8 raw, 5 served" across all 19 .html under sites/ — was
  // measured over BOTH roots even then; only the guard's domain has caught up.
  //
  ...
  // 🔴 `found`, NOT `found.filter(isPageQualitySubject)`, AND THAT IS UNCHANGED
  // FROM 2026-08-21: the box limb's domain is every served .html under the
  // page-quality roots, INCLUDING the pages the accessibility contract excuses.
  // A dated snapshot, `_template.html` and the mirror's `cv.html` are all served
  // bytes, and an unsized image reflows the page it is on whether or not that
  // page owes a skip link. The excuses above are about affordances the page does
  // not have; this is about bytes it does.
...
//
// ⚠️ WIDENED 2026-08-25, AND THE PARAGRAPH ABOVE IS LEFT STANDING BECAUSE THE
// DEFECT IT DESCRIBES WAS THE OPPOSITE WAY ROUND FROM WHAT IT ENFORCED. It says
// the two HAND-WRITTEN homepages "carried all four properties" while the
// generated landings carried one — so the limb was aimed at the generated half
// and the two pages it praises were asserted by NOTHING. `landings` was
// `[...files.keys()].filter(rel => rel.startsWith(APPS_DIR + '/'))`, and neither
// homepage is under APPS_DIR. Either could have lost og:image:width, or grown a
// declared size the artwork does not have, with this limb printing its count.
//
// The subject is now a LIST of {asset, pages}: one entry per page-quality root,
// each carrying that root's OWN og-image.png and its own hand-written
// index.html, plus — for DEPLOY_ROOT, the only root the generator writes into —
// the generated landings under APPS_DIR. Per-root and not one shared asset,
// because the two files are DIFFERENT files: measured 2026-08-25 from each
// IHDR, sites/nikatru/og-image.png and sites/rajasekarselvam/og-image.png are
// both 1200x630 but they are 118,197 and 46,622 bytes — the same trap M16 was
// corrected for on 2026-08-21, when one file's byte count was written as if it
// were both roots'. Nothing here retypes either number; both are read per run.
//
// The last clause of the paragraph above — "and the mirror deploy root" — is
// therefore RETIRED. What is still not caught is everything before it, plus one
// thing the widening ADDS and that is stated here rather than discovered later:
// a page is compared to the og-image.png OF THE ROOT IT IS SERVED FROM, not to
// whatever host its own `og:image` URL names. That is right for both roots today
// — each homepage points at its own origin, measured 2026-08-25 — and it is the
// only reading available without fetching, since the URL is absolute and this
// guard has no network. A page that pointed its card at the OTHER site's image
// would be compared to the wrong file and could be certified; nothing here
// catches that, and closing it means resolving the URL to a deploy root, which
// is a different limb than this one.
let ogChecked = 0;
/** asset path -> {w,h} read from that file's own IHDR, one entry per subject. */
const ogImagePx = new Map();
{
  ...
  // The generated half. APPS_DIR is inside DEPLOY_ROOT, which is why only that
  // root's subject carries these — the mirror is generated by nothing.
  const generatedLandings = [...files.keys()].filter((rel) => rel.startsWith(`${APPS_DIR}/`));
  const subjects = PAGE_QUALITY_ROOTS.map((root) => ({
    asset: `${root}/og-image.png`,
    pages: [...(root === DEPLOY_ROOT ? generatedLandings : []), `${root}/index.html`],
  }));

  for (const { asset, pages } of subjects) {
    const assetAbs = abs(asset);
    ...
    if (existsSync(assetAbs)) {
      ...
      if (head.length === 24 && head.subarray(12, 16).toString('latin1') === 'IHDR') {
        px = { w: head.readUInt32BE(16), h: head.readUInt32BE(20) };
      }
    }
    if (!px) {
      problems.push(
        `${asset} is missing or is not a PNG whose IHDR can be read, and ${pages.length} ` +
      ...
      ogImagePx.set(asset, px);
    }

    for (const rel of pages) {
      if (!existsSync(abs(rel))) {
        // A page the generator PLANS is limb A's MISSING message, naming the
        // file — restating it here is the two-guards-on-one-fault shape this
        // file refuses. A page it does NOT plan — the mirror's hand-written
        // homepage — is planned, diffed and missed by nothing else in this file,
        // so silence about it would be this limb's own blind spot.
        if (files.has(rel)) continue;
        problems.push(
          `${rel} does not exist, and it is a HAND-WRITTEN page this limb names explicitly: nothing else in ` +
            'this file plans it, diffs it or walks to it, so no other message will be produced about it. ' +
            'Either the site lost its homepage — served straight from this repository, so that is a 404 for ' +
            'every visitor — or this limb is pointed at a root that no longer keeps one.',
        );
        continue;
      }
      const html = readFileSync(abs(rel), 'utf8');
      ...
      if (px && w !== null && h !== null && (w !== String(px.w) || h !== String(px.h))) {
        problems.push(
          `${rel} declares og:image ${w}x${h} and ${asset} is actually ${px.w}x${px.h}. ` +
  ...
  // (Appended 2026-08-25: still true with the hand-written homepages in the
  // subject list. Each of them either declares no og:image and pushes its own
  // named problem, or is absent and pushes the explicit does-not-exist problem
  // above — there is no page in this limb's domain that can go uncounted and
  // unremarked.)
...
console.log(
  `ok  discovery surface — ${registry.length} registry entr(ies), ${live.length} live; ` +
    ...
    `resolves + :focus-visible + alt on every <img> (${pageExcusalsAudited} page(s) and ` +
    `${conditionExcusalsAudited} single condition(s) excused by name, each audited to still be needed); ` +
    `${boxChecked} served <img> tag(s) across ${PAGE_QUALITY_ROOTS.join(' + ')} reserve their box with integer width+height ` +
    '(tags inside comments and <script> are NOT elements and are not counted — the accessibility limb says why); ' +
    `${ogChecked} page(s) — ${[...files.keys()].filter((r) => r.startsWith(`${APPS_DIR}/`)).length} generated landing(s) plus ` +
    `${PAGE_QUALITY_ROOTS.length} hand-written homepage(s) — carry all four og:image properties, sized ` +
    ...
    //
    // (Appended 2026-08-25: `ogImagePx` is now a Map keyed by asset, one entry
    // per page-quality root, and the same argument carries — a subject whose
    // IHDR could not be read pushes a problem and never reaches `.set()`, so on
    // the success path this Map has exactly one entry per subject and there is
    // still nothing to fall back to. Each root's dimensions are printed against
    // that root's OWN file, because they are two different files that only
    // happen to agree today.)
    [...ogImagePx].map(([asset, px]) => `${px.w}x${px.h} from the IHDR of ${asset} itself`).join(', and '),
);
