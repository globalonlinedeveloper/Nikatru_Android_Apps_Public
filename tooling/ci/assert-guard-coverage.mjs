...
const NOT_A_SCANNER = new Map([
  ...
  [
    'dart-source.mjs',
    'is not a guard: it is the ONE reading of "which bytes of a .dart file are CODE" — the length-preserving `stripDartComments` and `blankDartStrings`, comments blanked so offsets survive, string literals kept in the first (a test NAME is a string literal) and blanked in the second (a brace inside a string is not a brace). Pure functions, text in, text out, no filesystem and no tree, so "did my scan still reach the tree" belongs to its importers, each of which carries its own COVERAGE LOST over what it read. It moved out of assert-app-dod.mjs on 2026-09-09, VERBATIM, when assert-mutation-proofs.mjs needed the same reading: that guard pins each mutation proof to a sha256 of the effect file with its comment prose stripped, and a hash computed by a SECOND stripper would disagree with the two limbs of assert-app-dod.mjs that already read this one — the effect anchor, which asks whether a symbol survives outside comments, and `lastCodeChangeDay`, which compares historical blobs through it. That is not a hypothetical risk here: `lastCodeChangeDay` EXISTS because those two limbs of that one guard disagreed with each other about what a Dart file\'s code is, and limb 3 expired five mutation rows over a commit that moved nothing but doc-comment prose. Two FILES disagreeing would be the same defect with a wider blast radius and nothing comparing them. Its own failing cases are in test/app-dod.test.mjs (32 cases, every one of which reaches the stripper through the guard) and test/mutation-proofs.test.mjs, which asserts specifically that a comment-only edit to an effect file does NOT expire a proof while a code edit does — the two directions the stripper is load-bearing for. It sits flat in tooling/ci because the stray-.mjs check above (correctly) treats a subdirectory as a guard escaping the scan.',
  ],
  [
    'workflow-scan.mjs',
  ...
  [
    ...
    'is not a guard: it is the ONE reading of "this Worker module was emptied into `services/_shared/src/`, and here is the file that now carries it" — the whole-file re-export rule, the on-disk resolution of the specifier, and the refusal to answer for a target outside the one shared home. [ADR 067] decision 2 gave `health.ts` and `error-sink.ts` one home each (they existed three times, at 333/333/334 and 200/196 lines with different hashes), so every path-pinned guard that read `services/<w>/src/lib/<m>.ts` now has to read the file that carries the body instead. It answers one of exactly three ways: `null` (no delegation, judge it where it is), `{ lost }` (it names the shared home and could not be followed — which the CALLER reports as COVERAGE LOST) and `{ target, source }`. That second channel IS the coverage report, raised to the caller instead of exited on, and it is not theoretical: run against the moved tree, assert-worker-error-sink.mjs\'s previous version reported four FAILs per Worker rather than a silent pass, which is the failure this module turns into an answer. It is pure functions plus the one file its caller names; it walks no directory and asserts nothing about any tree, so "did my scan still reach the tree" belongs to its importers, each of which already carries its own COVERAGE LOST over what it read. Its own failing cases are in test/worker-shared-modules.test.mjs, which mutates every refusal (a carrier that re-exports AND declares, a specifier resolving to no file, a target outside `services/_shared/src`, a delegation hidden in a comment) and fails when any of them starts answering `{ target }` — plus a green control, without which every one of those reds would be consistent with a resolver that refuses everything. It sits flat in tooling/ci because the stray-.mjs check above (correctly) treats a subdirectory as a guard escaping the scan.',
  ],
  [
    'text-reductions.mjs',
  ...
  [
    'single-threaded-relaunch.mjs',
    'is not a guard: it is the ONE way a guard in tooling/ci does its work with V8 background tasks OFF — `relaunchSingleThreaded` re-spawns the calling script with --single-threaded (execArgv, argv and exit status passed through) so that its exit cannot deadlock on a background compile waiting for a GC that can no longer run (nodejs/node#54918). assert-launcher-icons.mjs hung CI about twelve times that way AFTER printing its full verdict; the relaunch was written inline there on 2026-09-11 (#619) and lifted here, unchanged in behaviour, when the class sweep of the same day found the exposure in assert-elf-page-alignment.mjs, assert-listing-assets.mjs and assert-stamp-brand-assets.mjs — four inline copies drift in exactly what a green run cannot show, which of them still passes its arguments and status through. It scans nothing and asserts nothing about any tree. When the relaunch cannot start, or the working process is killed before a verdict, it hands that to the CALLER\'s own reporter, which frames it, and returns 2 for the caller to exit with — which is why the marker is in its importers\' code and not in its own. It never exits and never reads the process arguments itself (the caller passes them in), so assert-guards-refuse-empty classifies it as the library it is; its first version did both and was probed as a program that exited 0 over an empty tree (CI, 2026-09-11). Its own failing cases are in test/single-threaded-relaunch.test.mjs, and each importer pins the relaunch in its own test file by requiring `V8 background tasks: OFF`. It sits flat in tooling/ci because the stray-.mjs check above (correctly) treats a subdirectory as a guard escaping the scan.',
  ],
  [
    'install-pinned-tool.mjs',
    'is not a guard: it is the ONE way ci.yml\'s `security-scan` job obtains a pinned binary — gitleaks, zizmor, osv-scanner and trivy — with a BOUNDED RETRY over the download and a CHECKSUM-VERIFIED CACHE in front of it. It performs an action rather than scanning anything, in the same class as record-deployment.mjs, so there is no tree it could silently cover less of; what it CAN lose is the retry and the verification, and neither is left to prose. It exists because on 2026-09-12 the push run for 6be21b57 went red on `wget -q -O … gitleaks_8.30.1_linux_x64.tar.gz` (exit 4, network) and on nothing else: the scanner never installed, ci-gate recorded main as not green, and deploy-web REFUSED — a deploy stopped by one bad second against github.com. `wget -q -O` retries nothing, and these four steps were the only pinned downloads in .github/workflows that did not already use the repository\'s `curl … --retry 3 --retry-all-errors` idiom. Four near-identical `run:` blocks were the wrong home for that fix: what differs between copies of a shell step is exactly what a green run cannot show, so the URL, the archive kind and the member are DATA here and the behaviour is one code path. The cache is UNTRUSTED INPUT and is treated as such — an Actions cache entry is writable from any branch, so every artifact, restored or downloaded, has its sha256 compared against the digest in tooling/versions.json before it is placed, and an entry that does not match is deleted and re-downloaded; there is no path through `acquire` that returns unverified bytes. It also does not make a scanner optional: when every attempt fails it throws and the CLI exits 1 naming each attempt, because a secret scanner that could not be installed is a build that stops. Its own failing cases are in test/install-pinned-tool.test.mjs — a download that fails once and then succeeds (the 09-12 red), one that never succeeds, a bound that binds, a cache entry whose bytes are wrong, downloaded bytes whose digest is wrong, and a corpus check that no workflow fetches these four any other way, which is what stops the next hand from writing `wget -q -O` again. It sits flat in tooling/ci because the stray-.mjs check above (correctly) treats a subdirectory as a guard escaping the scan.',
  ],
]);
