...
// COPIED from `apps/subscriptiontracker` and one thing is broken per case. The house rule is
...
/**
 * 🔴 THE ONE SUBJECT FILE THAT IS NOT IN GIT, AND WHY IT NEEDS A FIXTURE.
 *
 * `apps/<app>/.flutter-plugins-dependencies` is GENERATED by `flutter pub get` and
 * IGNORED (apps/subscriptiontracker/.gitignore:30). It is the right thing for the GUARD to read —
 * it is what the Apple build actually links, so it is the only derivation of the
 * plugin set that cannot go stale — and the guard runs in `workspace_gate`, right
 * after `flutter pub get --enforce-lockfile`, where it exists.
 *
 * This TEST FILE is a different matter: ci.yml runs the whole `tooling/ci/test/*`
 * glob in the `platform` lane, which has no Flutter in it at all, so on a CI runner
 * the file is simply absent and every case here died on one assertion. That is not
 * a finding about the guard — it is this file depending on something git does not
 * carry.
 *
 * So: the REAL file is used whenever it exists (a developer machine, and preflight,
 * where the mutations therefore still range over the real tree exactly as intended),
 * and a committed fixture stands in when it does not.
 *
 * ⚠️ A FIXTURE THAT NOBODY COMPARES IS A FIXTURE THAT ROTS, and a stale one here
 * would make the plugin-set limb pass against a plugin list the build stopped
 * having. `the committed fixture still matches the real generated file` below is
 * that comparison; it runs wherever the real file exists and states plainly that it
 * cannot run on a Flutter-less runner.
 */
const PLUGIN_DEPS_REL = `apps/${APP}/.flutter-plugins-dependencies`;
const PLUGIN_DEPS_FIXTURE = join(CI_DIR, 'test', 'fixtures', 'subscriptiontracker.flutter-plugins-dependencies.json');
const realPluginDeps = () => join(REPO, PLUGIN_DEPS_REL);
const havePluginDeps = () => existsSync(realPluginDeps());
const pluginDepsSource = () => (havePluginDeps() ? realPluginDeps() : PLUGIN_DEPS_FIXTURE);
...
function tree(mutate) {
  ...
  for (const [from, to] of SUBJECT_FILES) {
    const src = from === PLUGIN_DEPS_REL ? pluginDepsSource() : join(REPO, from);
...
describe('assert-apple-privacy-manifest.mjs — the positive controls', () => {
  // 🔴 THESE RUN AGAINST tree(null), NOT AGAINST REPO, AND THE REASON IS NOT
  // COSMETIC. The guard reads exactly SUBJECT_FILES, so an unmutated copy of them
  // IS the real subject — and on a machine that has run `flutter pub get`, tree()
  // copies the REAL .flutter-plugins-dependencies, so this is the real tree.
  // Pointed at REPO instead, all three died on a Flutter-less CI runner, where the
  // generated file does not exist: a positive control that cannot run in CI is a
  // positive control CI does not have. The stronger check against the literal
  // repository is kept below, conditioned on the file being there.
  ...
  // The literal working tree, in place, with nothing copied — the one check that
  // proves the guard is green on the repository a human is actually holding. It
  // needs the generated plugin file, so it states plainly when it could not run
  // rather than reporting ok over a thing it did not do.
  test('the literal working tree passes, where the generated plugin file exists', (t) => {
    if (!havePluginDeps()) {
      t.diagnostic(
        `no ${PLUGIN_DEPS_REL} on this machine (generated, gitignored), so the guard was not run ` +
          'against the working tree in place. The copy-based controls above still ran.',
      );
      return;
    }
    const { code, out } = run(REPO);
    assert.equal(code, 0, `the literal working tree must pass, got:\n${out}`);
  });
});
...
// ─────────────────────────────────────────────────────────────────────────────
// THE FIXTURE MAY NOT DRIFT FROM THE THING IT STANDS IN FOR.
// ─────────────────────────────────────────────────────────────────────────────
test('the committed fixture still matches the real generated file', (t) => {
  if (!havePluginDeps()) {
    // Not a skip that hides a failure: on a Flutter-less runner there is nothing
    // to compare AGAINST, and saying so is the honest report. The comparison runs
    // on every developer machine and in preflight, which is where a stale fixture
    // would be introduced in the first place.
    t.diagnostic(
      `no ${PLUGIN_DEPS_REL} on this machine (it is generated and gitignored), ` +
        'so the fixture could not be compared to it here. Run `flutter pub get` to make this case real.',
    );
    return;
  }
  const real = JSON.parse(readFileSync(realPluginDeps(), 'utf8'));
  const fixture = JSON.parse(readFileSync(PLUGIN_DEPS_FIXTURE, 'utf8'));
  for (const platform of PLATFORMS) {
    const names = (o) => (o.plugins?.[platform] ?? []).map((x) => x.name).sort();
    assert.deepEqual(
      names(fixture),
      names(real),
      `the ${platform} plugin set in tooling/ci/test/fixtures/subscriptiontracker.flutter-plugins-dependencies.json ` +
        'has drifted from the real generated file. Re-copy it: the fixture exists so this test file ' +
        'can run where the generated file does not, and a fixture nobody compares is one that rots.',
    );
  }
});
