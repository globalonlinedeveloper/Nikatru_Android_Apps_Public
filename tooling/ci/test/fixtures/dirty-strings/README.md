...
`assert-no-hardcoded-strings.mjs` enforces on the brick template and on `apps/subscriptiontracker/lib`,
...
## Why it moved off `apps/subscriptiontracker` (2026-08-08)

That known-dirty tree used to be `apps/subscriptiontracker/lib`, which made the guard's coverage claim
hostage to a **product** decision. Subly's l10n retrofit — Phase 4 of
`Private/pre-minimal-2026-09-08:plans/subly-restamp-execution.md` — cleans exactly the literals the canary
...
A fixture owned by the guard cannot be cleaned by product work. `apps/subscriptiontracker/lib` was kept
