...
/// [pipeline 10]D-5 — the two Play LISTING graphics, generated by the stamp.
///
/// 🔴 WHY THE STAMP GENERATES THEM AND `render-play-graphics.mjs` DOES NOT.
/// That script is the right tool for an app with real brand art: it parses the
/// committed `app_icon_{foreground,background}.svg` and rasterises them through
/// headless Chrome, so a re-colour propagates. A FRESHLY STAMPED APP HAS NEITHER
/// — the brick writes PNG icon sources, not SVG, and stamping cannot require a
/// browser. Leaving the two files out instead is not an option: the register
/// requires them in every `android-play` tree, so the factory would hand app #2
/// a red build on its first CI run. So the stamp lays down derivable placeholders
/// in exactly the formats Play accepts, and the moment there is real art the
/// script re-derives both.
///
/// [assetSpecs] is the register's `graphicAssets.assets` block, passed in rather
/// than restated here: Google's dimensions and the opposite alpha requirements
/// are declared ONCE, in `tooling/channel-register.json`, next to the URL and
/// fetch date they were read from. A copy in this file would be the second
/// declaration and the first to drift — and the two formats are opposites
/// (feature graphic 24-bit NO alpha, icon 32-bit WITH alpha), so getting one
/// backwards produces a file that looks perfect and is rejected at upload.
///
/// Deterministic and model-free, like the icons: same `app_id` + `seed_hex`,
/// same bytes, so a re-stamp is byte-identical ([3]S-15) and [ADR 019]'s
/// NO-IP-PROMPTING rule is obeyed by construction.
List<String> writeStoreGraphics({
  required Directory storeDir,
  required String appId,
  required String seedHex,
  required Map<String, Object?> assetSpecs,
}) {
  final int seed = _parseHex(seedHex);
  final List<bool> cells = _markCells(appId);
  final _Rgb bg = _Rgb.fromInt(seed);
  ...
  final written = <String>[];
  for (final MapEntry<String, Object?> entry in assetSpecs.entries) {
    if (entry.key.startsWith('_')) continue; // `_why` prose, not an asset
    final spec = entry.value;
    if (spec is! Map) continue;
    final int? width = spec['width'] is int ? spec['width'] as int : null;
    final int? height = spec['height'] is int ? spec['height'] as int : null;
    // A spec with no dimensions cannot be honoured, and inventing one would
    // write a correct-looking file at a size nobody declared. Skip it: the
    // register is malformed, and assert-store-metadata.mjs is what says so.
    if (width == null || height == null || width <= 0 || height <= 0) continue;
    final bool alpha = spec['alpha'] == true;
    final path = '${storeDir.path}/${entry.key}';
    File(path).writeAsBytesSync(
      _renderPanel(
        ...
        fg: fg,
        cells: cells,
        // Of the SHORT side, so the mark is square and centred on a 1024x500
        // banner as well as on a 512 square. 0.72 matches the plain icons.
        markFraction: 0.72,
        alphaChannel: alpha,
      ),
    );
...
/// The same mark on a RECTANGULAR field, for the store listing graphics.
///
/// A separate function rather than a generalisation of [_renderIcon] on purpose:
/// `tooling/ci/assert-stamp-brand-assets.mjs` re-derives every icon BYTE FOR
/// BYTE in JavaScript and compares. Widening the icon renderer to take a
/// rectangle would put that guard's re-derivation and this function one edit
/// apart forever, and the failure would land on the icons — the assets that
/// already had a defect this repository paid for once.
///
/// [alphaChannel] emits RGBA with alpha 255 everywhere: OPAQUE, but 32-bit.
/// That is Play's app-icon requirement ("32-bit PNG (with alpha)") and it is not
/// the same thing as a TRANSPARENT field, which is what [_renderIcon]'s
/// `transparentBackground` produces for the Android adaptive foreground. A
/// transparent store icon would show the console's own backdrop through it.
Uint8List _renderPanel({
  required int width,
  required int height,
  required _Rgb bg,
  required _Rgb fg,
  ...
  final int channels = alphaChannel ? 4 : 3;
  final px = Uint8List(width * height * channels);
  for (int i = 0; i < width * height; i++) {
    final int o = i * channels;
    px[o] = bg.r;
    px[o + 1] = bg.g;
    px[o + 2] = bg.b;
    if (alphaChannel) px[o + 3] = 0xFF;
  }
  // Of the SHORT side: a fraction of the width would make the mark taller than
  // a 1024x500 banner and crop it top and bottom.
  final double mark = (width < height ? width : height) * markFraction;
  final double cell = mark / 5.0;
  final double originX = (width - mark) / 2.0;
  final double originY = (height - mark) / 2.0;
    ...
    for (int gx = 0; gx < 5; gx++) {
      ...
      for (int y = y0; y < y1 && y < height; y++) {
        for (int x = x0; x < x1 && x < width; x++) {
          if (x < 0 || y < 0) continue;
          final int i = (y * width + x) * channels;
          px[i] = fg.r;
          px[i + 1] = fg.g;
          px[i + 2] = fg.b;
          if (alphaChannel) px[i + 3] = 0xFF;
        }
      }
    }
  }
  return _encodePng(width, height, px, channels: channels);
}
